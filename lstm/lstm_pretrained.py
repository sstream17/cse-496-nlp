...
# Get the GloVe embeddings ready (https://keras.io/examples/nlp/pretrained_word_embeddings/)
embeddings_index = {}
...
model.add(LSTM(int(np.floor(dim)), return_sequences=True))
#model.add(tf.keras.layers.Dropout(0.2))
model.add(Bidirectional(LSTM(int(np.floor(dim)), return_sequences=True)))
#model.add(tf.keras.layers.Dropout(0.2))
model.add(Bidirectional(LSTM(int(np.floor(dim)), return_sequences=True)))
model.add(tf.keras.layers.Dropout(0.2))
model.add(Bidirectional(LSTM(int(np.floor(dim)))))
model.add(tf.keras.layers.Dropout(0.2))
