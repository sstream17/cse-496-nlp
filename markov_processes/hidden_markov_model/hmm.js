...
const lexicon = new natural.Lexicon(lang, defaultCategory, defaultCategoryCapitalized);
...
*/
const observedDict = new Object();
const hiddenDict = new Object();
    ...
    for (let i = 0; i < lyrics.length; i++) {
        if (isLyricLine(lyrics[i])) {
            ...
            for (let j = 0; j < tokens.length - 2; j++) {
                ...
                const pairPOS = tagger.tag(pair).taggedWords
                const word1 = pairPOS[0];
                const word2 = pairPOS[1];
                ...
                if (j == tokens.length - 3) {
                    const finalWord = tagger.tag([tokens[tokens.length - 1]]).taggedWords;
